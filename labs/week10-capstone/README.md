...
python -m netlab client stream-http --host 127.0.0.1 --port 8090 --path /stream?prompt=direct --show-headers
python -m netlab client http --host 127.0.0.1 --port 8081 --path /stream?prompt=capstone
python -m netlab client stream-http --host 127.0.0.1 --port 8081 --path /stream?prompt=capstone --show-headers
...
- Use `client stream-http` to print status, `X-Trace-Id`, `first_byte_ms`, per-chunk arrival timing, and `total_ms`.
